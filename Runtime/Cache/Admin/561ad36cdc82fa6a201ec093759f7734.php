...
<div class="layui-tab-brief main-tab-container">
    <ul class="layui-tab-title main-tab-title">
      <a href="<?php echo U('index');?>"><li>单页列表</li></a>
      <a href="<?php echo U('edit');?>"><li class="layui-this">添加单页</li></a>
      <div class="main-tab-item">单页管理</div>
    </ul>
      ...
      <div class="layui-tab-item layui-show">
        <form class="layui-form">
          <div class="layui-tab layui-tab-card">
            ...
            <div class="layui-tab-content">
              <div class="layui-tab-item layui-show">
                <?php echo Form::select('pid',$info['pid'],'上级分类','',$page_list,'title');?>
                <?php echo Form::input('text','title',$info['title'],'标题','（用于后台显示的配置标题）','请输入标题','required');?>
                <?php echo Form::input('text','tag',$info['tag'],'副标题');?>
                <?php echo Form::editor('content',$info['content'],'内容');?>
                <?php echo Form::radio('is_show',$info['is_show'],'是否显示','',array(1=>'是',0=>'否'));?>
                <?php echo Form::radio('recommend',$info['recommend'],'推荐','',array(1=>'是',0=>'否'));?>
                <!-- <?php echo Form::input('text','link',$info['link'],'转向链接','（外链如: http://www.baidu.com,內链如: :zxly以":"开头.在"链接模块"下有效）','请输入链接地址','');?> -->
                <?php echo Form::input('text','templete',array($info['templete'],'page'),'模版名','（前台显示页面）','请输入模版名','');?>
                <?php echo Form::input('text','html',$info['html'],'静态名','（设置栏目在前台页面显示的静态文件名称，无需添加扩展名，常常以栏目名称拼音命名，留空则取栏目名称拼音首字母作为页面名称！如：gongsijianjie/gsjj）','请输入静态名','');?>
                <?php echo Form::input('text','sort',array($info['sort'],100),'排序','','数字越小越靠前','number');?>
              </div>
              <div class="layui-tab-item">
                <?php echo Form::input('text','seo_title',$info['seo_title'],'SEO标题','（用于后台显示的配置标题）','请输入标题');?>
