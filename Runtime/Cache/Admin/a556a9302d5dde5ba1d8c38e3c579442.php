...
<div class="layui-tab-brief main-tab-container">
    <ul class="layui-tab-title main-tab-title">
      ...
      <a href="<?php echo U('edit');?>"><li class="layui-this">添加 / 编辑产品</li></a>
      <a href="<?php echo U('category');?>"><li>产品分类列表</li></a>
      <a href="<?php echo U('category_edit');?>"><li>添加 / 编辑产品分类</li></a>
      ...
      <div class="layui-tab-item layui-show">
        <form class="layui-form">
          <div class="layui-tab layui-tab-card">
            ...
            <div class="layui-tab-content">
              <div class="layui-tab-item layui-show">
                <?php echo Form::select('catid',$info['catid'],'分类','',$category,'title');?>
                <?php echo Form::input('text','title',$info['title'],'标题','（用于后台显示的配置标题）','请输入标题','required');?>
                <?php echo Form::upload('img', $info['img'], '图片', '列表缩略图');?>
                <?php echo Form::input('text','sn',$info['sn'],'货号','','','');?>
                <?php echo Form::input('text','market_price',$info['market_price'],'市场价','','','');?>
                <?php echo Form::input('text','discount',$info['discount'],'折扣','（使用小数）','','');?>
                <?php echo Form::input('text','price',$info['price'],'价格','','','');?>
                <?php echo Form::input('text','size',$info['size'],'尺寸','','','');?>
                <?php echo Form::input('text','stock',$info['stock'],'库存','','','');?>
              ...
              <div class="layui-tab-item">
                <?php echo Form::upload('imgdata', $info['imgdata'], '图片', '列表缩略图','products',true);?>
              </div>
              <div class="layui-tab-item">
                ...
                <?php echo Form::input('text','keywords',$info['keywords'],'关键词','设置页面关键词，留空表示继承上级栏目设置！建议在多个关键词之间以英文逗号","分隔，并且最好不要超过100个字符！如：软件开发,网站建设','','');?>
                <?php echo Form::textarea('description', $info['description'], '描述', '（设置页面描述，留空表示继承上级栏目设置！建议不要超过200个字符！）');?>
              </div>
              <div class="layui-tab-item">
                ...
                <?php echo Form::input('text','click',$info['click'],'点击数','','','');?>
