...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        fns.onload || (fns.onload = function (resultText, xhr) {
            var Text = eval("("+resultText+")");
            E.log('上传结束，返回结果为 ' + Text.data);
            ...
            if (Text.status === 0) {
                // 提示错误
                E.warn('上传失败：' + Text.data);
                alert(Text.data);
            } else {
                E.log('上传成功，即将插入编辑区域，结果为：' + Text.data);

                // 将结果插入编辑器
                img = document.createElement('img');
                img.onload = function () {
                    var html = '<img src="' + Text.data + '" alt="' + originalName + '" style="max-width:100%;" data-info="wE_upload" />';
                    editor.command(null, 'insertHtml', html);

                    E.log('已插入图片，地址 ' + Text.data);
                    img = null;
                };
                img.onerror = function () {
                    E.error('使用返回的结果获取图片，发生错误。请确认以下结果是否正确：' + Text.data);
                    img = null;
                };
                img.src = Text.data;
            }
